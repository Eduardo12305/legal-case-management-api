...
router.post('/', authorizeRoles('ADMIN', 'LAWYER'), validateRequest({ body: createProcessBody }), asyncHandler((req, res) => processController.create(req, res)));
...
router.put('/:id', authorizeRoles('ADMIN', 'LAWYER'), validateRequest({ params: processIdParams, body: updateProcessBody }), asyncHandler((req, res) => processController.update(req, res)));
router.patch('/:id/status', authorizeRoles('ADMIN', 'LAWYER'), validateRequest({ params: processIdParams, body: updateStatusBody }), asyncHandler((req, res) => processController.updateStatus(req, res)));
...
router.post('/:id/documents', authorizeRoles('ADMIN', 'LAWYER'), validateRequest({ params: processIdParams, body: addDocumentBody }), asyncHandler((req, res) => processController.addDocument(req, res)));
router.post('/:id/updates', authorizeRoles('ADMIN', 'LAWYER'), validateRequest({ params: processIdParams, body: addUpdateBody }), asyncHandler((req, res) => processController.addUpdate(req, res)));
